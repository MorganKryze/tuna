...
// sshPath is a variable rather than a literal so a test can point tunny at a
...
func sshRunner(ctx context.Context, d *config.Destination, up func()) tunnel.Result {
	...
	// tunny killed tunny alone: ssh was reparented to init and kept holding the
	// forwarded ports, so the next run reported them busy and told the
	// operator to go find the culprit with lsof. The culprit was tunny's own
	// child.
	//
	...
	// grandchild outliving its parent would hang tunny with no output and no
	...
	// covers a signal tunny received; signaled() covers the terminal delivering
	// Ctrl-C straight to the foreground group, where ssh dies before tunny has
	// looked at anything.
