...
// fakeSSH puts a shell script where tunny looks for ssh. The whole point of
...
// The defect this test exists for: a SIGTERM to tunny used to kill tunny alone.
// ssh was reparented to init and kept holding the forwarded ports, so the next
// run reported them busy and pointed the operator at lsof to find tunny's own
// child.
//
...
// here is tunny taking the child down itself.
func TestCancellingTheContextTakesTheChildWithIt(t *testing.T) {
	...
	t.Fatalf("ssh (pid %d) survived tunny: it is an orphan holding the local ports", pid)
}
...
// A child killed by SIGTERM is somebody asking tunny to stop, and must not
